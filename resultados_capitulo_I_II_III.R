###
# _ Resultados obtidos no capitulo I ----
...
m1  %>%
  ...
  labs(y = "Percentual de receita prÃ³pria", x = "Tamanho Populacional") +
...
m1  %>%
  ...
  geom_jitter(adjust = 1L, scale = "area", fill = "#0c4c8a", alpha= 0.7) +
  labs(y = "Percentual de receita prÃ³pria", x = "Regiao")+
  theme_minimal()
...
###
# _ Resultados Obtidos no capitulo II ----
...
partido <- ggplot(partidos) +
  aes(x = reorder(Decada, o), weight = `NÃºmero total de Partidos`) +
  geom_bar(fill = "#8E9094") +
  theme_bw() +
  #theme_minimal()+
  labs(x = "DÃ©cada", y = "NÃºmero total de partidos Politicos")


ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_II/partido.png", width = 20, height = 15, units = "cm")

###
# _ Resultados Obtidos no capitulo III ----

###
# Pacotes Utilizados no capitulo III -----

load("codigos_dissertacao/banco_de_dados/banco_dissertacao_pronto.Rda")
load("G:/Meu Drive/Acadêmica/meus.trabalhos.maiores/dissertacao/banco_de_dados/banco_dissertacao_pronto.Rda") # Cerrega no pc I5

# Numero de Cadeiras conquistadas por partido em 2004, 2008, 2012, 2020 ----
###
...
# Pronto está numa tabeal do excell
...
# R.Boxplot. Variavel dependente, regiao por ciclo   ----
### 
  ...
  vari_dependente_reg_ciclo <-  vari_regi %>%
    filter(!(regiao %in% ""), Media_Gasto_Saude <= 2000) %>%  # Sem outlier
    ggplot() +
    aes(x = regiao, y = Media_Gasto_Saude) +
    geom_boxplot() +
  ...
  ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/vari_dependente_reg_ciclo.png", width = 20, height = 15, units = "cm")  

  ggsave("C:/Users/Use/Desktop//vari_dependente_reg_ciclo.png", width = 20, height = 15, units = "cm")
...
### 
  ...
  mutate (Ciclos = case_when(Ciclos == 'gasto_medio_05_08' ~ 'Ciclo 2005 a 2008',
          ...
          Ciclos == 'gasto_medio_13_16' ~ 'Ciclo 2013 a 2016',
          Ciclos == 'gasto_medio_17_20' ~ 'Ciclo 2017 a 2020'), 
          cat_12_n = case_when(pop_cat_2012   == "atÃ© 5 mil habitantes" ~ 1,
          pop_cat_2012 == "entre 5 e 10 mil habitantes" ~ 2,
          pop_cat_2012 == "entre 10 e 20 mil habitantes" ~ 3,
          pop_cat_2012 == "acima de 500 mil  habitantes" ~ 7,
          pop_cat_2012 ==  "entre 100 e 500 mil habitantes" ~ 6,
  ...
  # Salvar
    ...
    variavel_dependente_ciclo_porte <- vari_pop %>%
    filter(!(pop_cat_2012 %in% ""), Media_Gasto_Saude <= 1500) %>% # outliner 
    filter(!is.na(cat_12_n)) %>%
    ggplot() +
    aes(x = fct_reorder(pop_cat_2012, cat_12_n), y = Media_Gasto_Saude) +
   ...
   ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_ciclo_porte.png", width = 20, height = 15, units = "cm")  
...
### 
  ...
  long_saude_partido <- banco_dissertacao_pronto %>% 
    ...
    # Salvar 
      ...
      variavel_dependente_ciclo_partido <- long_saude_partido %>%
      filter(!(partido%in% ""),gasto <= 2000) %>% # outliner 
      filter(!is.na(partido)) %>%
      ggplot() +
      aes(x = fct_reorder(partido, media_partido), y = gasto) +
    ...
    ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_ciclo_partido.png", width = 20, height = 15, units = "cm")  
...
# Salvar 
    variavel_dependente_ideologia_ciclo_partido <-  long_saude_ideologia %>%
      filter(!(ideologia_d_e_c%in% ""),gasto <= 2000) %>% # outliner 
      filter(!is.na(ideologia_d_e_c)) %>%
      ggplot() +
      aes(x = fct_reorder(ideologia_d_e_c, media_ideologia), y = gasto) +
    ...
    ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_ideologia_ciclo_partido.png", width = 20, height = 15, units = "cm")      
        
# R.Boxplot.Variavel Dependente no ultimo ciclo por ideologiasegundo a regiao----
## variavel_dependente_ideologia_ultimo_ciclo_regiao_partido
...
# Salvar    
    ...
    variavel_dependente_ideologia_ultimo_ciclo_regiao_partido <-     long_saude_ideologia_regiao %>%
      filter(!(ideologia_d_e_c%in% ""),gasto <= 2000) %>% # outliner 
      filter(!is.na(ideologia_d_e_c)) %>%
      ggplot() +
      aes(x = fct_reorder(ideologia_d_e_c, media_ideologia), y = gasto) +
    ...
    ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_ideologia_ultimo_ciclo_regiao_partido.png", width = 20, height = 15, units = "cm")      
      
# R.Boxplot. Variavel Dependente por primeiro mandato segundo o ciclo ----
## variavel_dependente_primeiro_mandato_ciclo_partido
    ...
    long_saude_primeiro_mandato %>%
      ...
      theme_bw() +
      stat_summary(fun=mean, geom="point", shape=20, size=2, color="blue", fill="blue") +
      labs(x = "Situação do Prefeito", y = "Media do Gasto Proprio per capita em Saude") 
   
# Salvar     
     
    
    variavel_dependente_primeiro_mandato_ciclo_partido <- long_saude_primeiro_mandato %>%
      filter(!(primeiro_mandato_e%in% ""),gasto <= 2000) %>% # outliner 
      filter(!is.na(primeiro_mandato_e)) %>%
      ggplot() +
      aes(x = fct_reorder(primeiro_mandato_e, media_primeiro_mandato), y = gasto) +
    ...
    ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_primeiro_mandato_ciclo_partido.png", width = 20, height = 15, units = "cm")
    
# R.Boxplot. Gasto no úlitmo Ano de Governo por primeiro mandato----
## variavel_dependente_primeiro_mandato_ulitmo_ano_partido
    ...
    long_saude_primeiro_mandato_ano %>%
      ...
      theme_bw() +
      stat_summary(fun=mean, geom="point", shape=20, size=2, color="blue", fill="blue") +
      labs(x = "Situação do Prefeito", y = "Gasto medio per capita em Saude") 
    
    # Salvar 
    
    variavel_dependente_primeiro_mandato_ulitmo_ano_partido <-     long_saude_primeiro_mandato_ano %>%
      filter(!(primeiro_mandato_e%in% ""),gasto <= 2000) %>% # outliner 
      filter(!is.na(primeiro_mandato_e)) %>%
      ggplot() +
      aes(x = fct_reorder(primeiro_mandato_e, media_primeiro_mandato), y = gasto) +
    ...
    ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_primeiro_mandato_ulitmo_ano_partido.png", width = 20, height = 15, units = "cm")
    
# R.Boxplot. Variavel Dependente por profissional de saude segundo o ciclo ----
## variavel_dependente_prof_saude_ciclo
    ...
    long_prof_saude %>%
      filter(!(prof_saude_e%in% ""),gasto <= 2000) %>% # outliner 
      ...
      theme_bw() +
      stat_summary(fun=mean, geom="point", shape=20, size=2, color="blue", fill="blue") +
      labs(x = "Profissão do Prefeito", y = "Media do Gasto Proprio per capita em Saude") 

    # Salvar 
    
    variavel_dependente_prof_saude_ciclo <- long_prof_saude %>%
      filter(!(prof_saude_e%in% ""),gasto <= 2000) %>% # outliner 
      filter(!is.na(prof_saude_e)) %>%
      ggplot() +
      aes(x = fct_reorder(prof_saude_e, media_prof_saude), y = gasto) +
    ...
    ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_prof_saude_ciclo.png", width = 20, height = 15, units = "cm")
    
# R.Boxplot. Variavel Dependente por maioria parlamentar segundo o ciclo ----
## variavel_dependente_maioria_parlamentar_ciclo
       ...
       maioria %>%
       filter(!(maioria_e%in% ""),gasto <= 2000) %>% # outliner 
       ...
       theme_bw() +
       stat_summary(fun=mean, geom="point", shape=20, size=2, color="blue", fill="blue") +
       labs(x = "Prefeito com Maioria e Sem Maioria", y = "Media do Gasto Proprio per capita em Saude") 
    
       # Salvar 
       
       variavel_dependente_maioria_parlamentar_ciclo  <-   maioria %>%
         filter(!(maioria_e%in% ""),gasto <= 2000) %>% # outliner 
         filter(!is.na(maioria_e)) %>%
         ggplot() +
         aes(x = fct_reorder(maioria_e, media_maioria), y = gasto) +
       ...
       ggsave("C:/Users/Larissa/codigos_dissertacao/resultados/imagem_capitulo_III/variavel_dependente_maioria_parlamentar_ciclo.png", width = 20, height = 15, units = "cm")
